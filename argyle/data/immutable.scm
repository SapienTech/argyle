...
(define-syntax-rule (define-immutable-record-type name ctor pred fields ...)
  ((@@ (argyle data records) %define-record-type)
...
(define-syntax %set-fields
  (lambda (x)
    (with-syntax ((getter-type   #'(@@ (argyle data records) getter-type))
                  (getter-index  #'(@@ (argyle data records) getter-index))
                  (getter-copier #'(@@ (argyle data records) getter-copier)))
      (syntax-case x ()
